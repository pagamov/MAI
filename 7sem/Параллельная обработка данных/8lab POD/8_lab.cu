...
__global__ void kernel1(double* next, double* data, int blockX, int blockY, int blockZ, double hx, double hy, double hz){
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < blockX; i += blockDim.x * gridDim.x) {
        for (int j = blockIdx.y * blockDim.y + threadIdx.y; j < blockY; j += blockDim.y * gridDim.y) {
            for (int k = blockIdx.z * blockDim.z + threadIdx.z; k < blockZ; k += blockDim.z * gridDim.z) {
...
__global__ void kernel2(double* next, double* data, int blockX, int blockY, int blockZ){
    ...
    int i, j, k;
    for (int i = idx - 1; i <= blockX; i += offsetx ){
        for (int j = idy - 1; j <= blockY; j += offsety ){
            for (int k = idz - 1; k <= blockZ; k += offsetz ){
